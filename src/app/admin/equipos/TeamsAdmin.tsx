...
import { createTeam, deleteTeam, addTeamPlayer, removeTeamPlayer } from '../actions'
...
export default function TeamsAdmin({ teams, regions, categories, clubs, players, rosters, seasonId }:
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const r = await createTeam(fd); setMsg(r.message); setBusy(false)
    if (r.ok) e.currentTarget.reset()
  }
  return (
    <div className="grid gap-6 lg:grid-cols-[380px_1fr]">
      <form onSubmit={submit} className="card space-y-3 p-5">
        <h3 className="font-bold">Nuevo equipo</h3>
        <input name="name" className="input" placeholder="Nombre del equipo *" required />
        ...
        <select name="category_id" className="input" required><option value="">Categoría…</option>
          {cats.map(c => <option key={c.id} value={c.id}>{c.name} · {c.gender}</option>)}</select>
        <select name="region_id" className="input" required><option value="">Región…</option>
          {regions.map(r => <option key={r.id} value={r.id}>{r.name}</option>)}</select>
        <select name="club_id" className="input"><option value="">Club (opcional)…</option>
          {clubs.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
        <input name="colors" className="input" placeholder="Colores (ej. Azul / Blanco)" />
        {msg && <p className="rounded-lg bg-slate-100 p-2 text-sm">{msg}</p>}
        <button disabled={busy} className="btn-primary w-full">{busy?'Guardando…':'Crear equipo'}</button>
      </form>
...
function TeamCard({ team, roster, players, setMsg }:
  { team: any; roster: any[]; players: Player[]; setMsg: (m:string)=>void }) {
    ...
    <div className="card p-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={async()=>{ if(!confirm('¿Eliminar equipo?'))return; setMsg((await deleteTeam(team.id)).message)}}
          className="rounded-lg px-2 py-1 text-xs font-semibold text-rose-500 ring-1 ring-rose-200 hover:bg-rose-50">Eliminar equipo</button>
      </div>
